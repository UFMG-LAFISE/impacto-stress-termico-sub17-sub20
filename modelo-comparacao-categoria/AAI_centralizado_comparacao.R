...
# dados aceleracao e desaceleracao
dados$AAI <- (dados$aceleracao_3_ms + dados$desaceleracao_3_ms) / dados$duracao_total_min
...
modelo_1 <- lmer(
  `AAI` ~ 
    ...
    posicao +
    IBUTG_med_c * posicao + # COLINEARIDADE
    (1 | atleta), 
  data = dados
)
...
tab_model(
  ...
  dv.labels = c("modelo 1 AAI", "modelo 2 AAI"),
