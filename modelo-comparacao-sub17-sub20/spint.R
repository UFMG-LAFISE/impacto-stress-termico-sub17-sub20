...
# modelo -----------------------------------------------------------------------
# sprint_25_kmh e uma CONTAGEM (nº de sprints > 25 km/h no jogo).
# Modelo binomial negativo com offset log(duracao_total_min) -> modela a taxa
# (sprints/min) respeitando a natureza discreta e a exposicao variavel.
# NAO se usa sprint_25_kmh_min (taxa) como resposta gaussiana.
modelo_1 <- glmmTMB(
  sprint_25_kmh ~
    IBUTG_med_c +
    categoria +
...
tab_model(
  ...
  dv.labels = c("modelo 1 Sprint (NB2, offset log duracao)"),
  string.pred = "Preditores",
  string.est = "IRR (exp Beta)",
  title = "Tabela 1. Modelo binomial negativo para nº de sprints e variáveis ambientais"
)
##################################################################
