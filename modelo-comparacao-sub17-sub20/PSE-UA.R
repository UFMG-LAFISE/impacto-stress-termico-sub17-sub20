...
# modelo ----------------------------------------------------------------------
# PSE_UA = PSE (escala de Borg CR10, aqui concentrada em 7-10) x duracao.
# Composto ordinal x tempo, levemente assimetrico. O LMM gaussiano foi o que
# apresentou melhor diagnostico DHARMa (KS e dispersao ok; leve desvio nos
# quantis). Gamma(log) piora. Alternativa formal: cumulative link mixed model
# (ordinal::clmm) na PSE bruta com duracao como covariavel.
modelo_1 <- lmer(
  `PSE_UA` ~
    IBUTG_med_c +
    categoria +
...
tab_model(
  ...
  dv.labels = c("modelo 1 PSE_UA (LMM)"),
  string.pred = "Preditores",
  string.est = "Estimativa (Beta)",
  title = "Tabela 1. Modelo misto para carga interna (PSE_UA) e variáveis ambientais"
)
##################################################################
