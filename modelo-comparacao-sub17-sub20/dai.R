# regressão 2 - distância em alta intensidade (U17 vs. U20)
...
# Modelo------------------------------------------------------------------------
# DAI = soma_3_valoc19.8kmh = DISTANCIA (metros) percorrida acima de 19,8 km/h.
# Variavel continua positiva e assimetrica a direita -> GLMM Gamma com link log
# e offset log(duracao_total_min), modelando a distancia por minuto.
# NAO se modela soma_3_valoc19.8kmh_min (taxa) como resposta gaussiana.
modelo_1 <- glmmTMB(
  soma_3_valoc19.8kmh ~
    IBUTG_med_c +
    categoria +
...
tab_model(
  ...
  dv.labels = c("modelo 1 DAI (Gamma log, offset log duracao)"),
  string.pred = "Preditores",
  string.est = "exp(Beta) (razao)",
  title = "Tabela 1. Modelo Gamma para distância em alta intensidade e variáveis ambientais"
)
##################################################################
